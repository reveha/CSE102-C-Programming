...
int has_loop(int *seq, int seqlen, int looplen, int *ls, int *le){
	...
	int index=0;
	int index2=index+looplen;
	int temp_seqlen=seqlen;
	int temp_index=index;
	int temp_index2=index2+looplen;
	int array1[looplen];
	int array2[looplen];
	int i,j,k,t;
	int check=1;
	while(index<temp_seqlen-(2*looplen)+1){
		//printf("%d\n",temp_seqlen-(2*looplen)+1);
		for(k=0;k<looplen;k++){
		...
		//printf("%d\n",seqlen-looplen*(2)+1);
		for(i=0;i<seqlen-looplen*(2)+1;i++){
		
			temp_index2=index2+looplen;
			k=0;
			for(j=index2;j<temp_index2;j++){
				array2[k]=seq[j];
				//printf("array2[%d]: %d\n",k,array2[k]);	
				k++;
			}
