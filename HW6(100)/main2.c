...
void printMap(){
	...
	int game_area[ROW][COLUMN]; // i assigned the ascii table values into the this array and i printed it with %c !!(i assume that string are not followed to use)
	int i,j;
	...
	do{
		...
		getchar();
		who_start=startGame(); // 1 means player 1 start, 2 means player 2 start
		printf("DICE: %d\n",who_start);
		printf("\033[0;34mPLAYER 2... press ENTER to dice");
		getchar();
		who_start2=startGame(); // 1 means player 1 start, 2 means player 2 start
		printf("DICE: %d\n",who_start2);
